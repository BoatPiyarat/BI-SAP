...
BEGIN
  ...
  SELECT
    ...
    s.OrderDate,COALESCE(NULLIF(TRIM(s.InsuredID),''),'-') InsuredID,IFNULL(s.Title,'') Title,
    s.FirstName,IFNULL(s.LastName,'') LastName,
    s.InsurerCode,s.InsuranceGroup,s.InsuranceType,s.InsuranceProduct,s.ProductType,s.PolicyType,
    s.Endorse,s.PolicyDate,s.PolicyNo,IFNULL(s.EndorsementNo,'') EndorsementNo,
    IFNULL(s.ChassisNo,'') ChassisNo,IFNULL(s.LicensePlate,'') LicensePlate,
    ...
    IFNULL(s.RefOrder,'') RefOrder,FORMAT('%.2f',IFNULL(s.RefundAmountBeforeFee,0)) RefundAmountBeforeFee,
    FORMAT('%.2f',IFNULL(s.RefundAmountAfterFee,0)) RefundAmountAfterFee,s.BillingAddress,
