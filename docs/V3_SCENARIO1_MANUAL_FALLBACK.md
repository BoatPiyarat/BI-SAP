...
2. Make a temporary copy of `sql/operator/20260826_export_v3_onetime_create_manual.sql`, replace
   ...
   quoting when required, and CRLF or LF record separators accepted by the reviewed parser. Replace
...
3. Record the row count and SHA-256 of the local file. Compare its row count with the query result
   and retain the run ID, commit, operator, timestamp, and hash in the delivery manifest.
4. **Current fail-closed gate:** do not upload this Scenario 1 file yet. The deployed
...
5. After that marker is deployed, a human with current session authority may copy the exact reviewed
...
6. Ingest the exact SAP result using `docs/design/SAP_RUNBOOK_v3.md` sections 6.3–6.4, then call
...
7. If pickup has no independently matched result by the timeout defined in
...
8. If any assertion, hash, count, pickup, import, or ACK check differs, stop. Preserve the file and
