-- Read-only reconciliation for linked change-order cancellation preparation.
SELECT
...
FROM `pacific-plating-282708.sap_integration_v3.v3_change_order_cancel_payload_summary` AS summary
LEFT JOIN (
  SELECT ownership_run_id, hold_code, COUNT(*) AS item_count,
    SUM(payload_row_count) AS payload_rows
  FROM `pacific-plating-282708.sap_integration_v3.v3_change_order_cancel_payload_hold`
