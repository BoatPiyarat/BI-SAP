-- CLASS A / PRODUCTION DDL — DO NOT RUN WITHOUT CLASS-A PASS + SEPARATE BOAT DEPLOY APPROVAL.
-- One-time legacy-view exception: docs/AGENT_RULES.md (Boat, 2026-08-24).
-- Fresh live source: pacific-plating-282708.sap_data_engineer.sap_dashboard_carepay_installment
-- Captured: 2026-08-24 ICT; live lastModifiedTime: 1786025613958
-- Live query SHA-256 (UTF-8): fa0a557260d99083b9bd3af1aa2a09d5f55828d8c15088235ff7a3cd071f1aa7
-- Candidate query SHA-256 (UTF-8): 148813fb8035046a456a090b0cc76c982dc6b52fec0a89ae9bdad72fef2cf163
-- Scope: only the reviewed InvoiceNo NULL-safety predicates from a3d0305 + 0ac324f.
-- The compulsory-installment predicate intentionally remains bare and unchanged.

CREATE OR REPLACE VIEW `pacific-plating-282708.sap_data_engineer.sap_dashboard_carepay_installment` AS
WITH
charges AS (
  SELECT * ,
...
CASE WHEN charges.installment_number = 1 THEN CONCAT('2_',COALESCE(charges.third_party_id,order_items.human_id))
  ...
  WHEN charges.third_party_id is null AND COALESCE(charges.status,'') <> 'SUCCESSFUL' THEN ''
...
transformation AS (
  SELECT
    ...
    OrderID,
    OrderItem,
    CASE WHEN InvoiceNo IS NULL AND COALESCE(TransactionStatus,'') <> 'SUCCESSFUL' THEN '' ELSE InvoiceNo END AS InvoiceNo,
