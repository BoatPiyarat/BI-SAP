...
-- Object: sap_data_engineer.RCL_MOTOR
...
get_charges_ref AS (
  ...
  -- and third_party_id ='chrg_5zgpwynh73bcl4u3dcr'
  -- order by charges.create_time desc limit 88;
...
rcb_voluntary_installment_details AS (
  ...
  WHERE 
    ...
    -- AND CONCAT('leads/',orders.lead_id) NOT IN (SELECT lead_resource FROM contracts)
    AND (follow_ups.transaction_id IS NULL)
    
    --AND orders.is_cancelled IS FALSE
    --AND transaction_snapshots.is_current IS TRUE
    /* TEST CASE */
    -- AND transactions.status = 'SUCCESSFUL' AND follow_ups.status != 'FOLLOWUP_STATUS_PAID'
),
--------------------------------------------------------------------------------------------------------
...
rcl_voluntary_installment_details AS (
  ...
  WHERE 
    transaction_snapshot_installment_details.id IS NOT NULL
    AND (order_items.motor_item_type != 'MOTOR_TYPE_COMPULSORY' OR order_items.motor_item_type IS NULL)  -- A2 fix 2026-07-24
    -- AND CONCAT('leads/',orders.lead_id) IN (SELECT lead_resource FROM contracts)
...
voluntary_onetime_no_installment_details AS (
  SELECT
    ...
    order_items.approval_status AS ApprovalStatus,
    transactions.status AS PaymentStatus,
    -- ROUND((1 / 100) * charges.amount-transaction_snapshot_installment_details.add_ons,2) AS ActualReceived,
    CASE 
    ...
    -- ROUND((1 / 100) * charges.amount,2) AS ActualReceived,
    CASE 
      WHEN transaction_snapshot_price_summaries.interest_amount = 0 OR transaction_snapshots.number_of_installment - 1 = 0 THEN 0
      ELSE ROUND((ROUND((1 / 100) * transaction_snapshot_price_summaries.interest_amount,2) - ((ROUND((1 / 100) * transaction_snapshot_price_summaries.interest_amount,2) * 3.3) / 103.3)) / (transaction_snapshots.number_of_installment - 1),2)
    END AS InterestThisPeriod,
    ...
    CASE
      ...
      ELSE ROUND((1 / 100) * transactions.amount,2) -- ROUND((1 / 100) * ROUND((1 / 100) * charges.amount,2),2)
  ...
  WHERE 
    ...
    --AND transaction_snapshots.is_current IS TRUE
    /* TEST CASE */
    --AND transactions.status != 'SUCCESSFUL' AND charges.status = 'SUCCESSFUL' /* Doesn't make sense */
    --AND transactions.status = 'SUCCESSFUL' AND charges.status != 'SUCCESSFUL' /* It's must have new charges with status = 'SUCCESSFUL' */
    --AND charges.installment_number > 1 /* Doesn't make sense */
),
--------------------------------------------------------------------------------------------------------
...
compulsary_onetime_no_installment_details AS (
  SELECT
    ...
    order_items.approval_status AS ApprovalStatus,
    transactions.status AS PaymentStatus,
    order_items.price AS ExpectedReceived,   
