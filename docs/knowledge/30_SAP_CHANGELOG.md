...
- Added a Node VM test for exact LogID 21183 metadata/sender parsing, TXT detail parsing,
  SAP-result-name manifest match, production-name non-match, ambiguous-manifest refusal, ordered
  persist/publish/label, zero-manifest no-label, and ambiguity alert/failure behavior.
- All 18 assertions passed; Apps Script syntax and manifest JSON parsing also passed.
