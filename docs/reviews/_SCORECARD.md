...
| Claude Code | 32 | 1 | 1 (043 — both items reproduced/confirmed, fixes proposed, resolved by author in one round: `6ef690b`) — but see rework column | Stale-claim timing risk in `0c74639`; missing-attachment fallback gap in `eb93ef6`; independently reproduced `e02bd39`'s daily-loss table byte-for-byte rather than only reading it; metadata-exposure claim vs verified `secretKeyRef` state + stale rotation status in `935ac8f`; 043 analytic-in-aggregate watermark + TIMESTAMP/DATE mismatch (both CALL-time, invisible to dry-run) | **1 — `6ef690b` re-review passed "DATE chain complete" while 4 WHERE predicates (8 comparisons) still compared TIMESTAMP vs DATE; caught by Boat, confirmed hard CALL-time error by standalone dry-run; Codex reworking** | 2 | Artifact review; self-caught Gmail threading and missing dry-run; one review (`e02bd39`) corroborated via a query already run for unrelated same-session work; 2026-08-01 batch used zero billed bytes total (4 schema metadata calls + 1 standalone 0-byte dry-run repro + offline self-test re-run) |
