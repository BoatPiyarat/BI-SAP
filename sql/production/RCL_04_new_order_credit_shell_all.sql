...
-- Object: sap_integration_v2.`RCL 04_new order credit shell_all`
...
-- 26/05/2025 Piyarat - updated multiple charge and actual received from multiple charge
-------------------------------------------------------------------------------------------------------------------------
WITH
get_charges_ref AS (
...
multic AS (SELECT transaction_id
FROM `pacific-plating-282708.careos.carepay_charges`
GROUP BY transaction_id 
HAVING count(third_party_id) >1
--
 ),
card_tokens AS (
...
rcb_voluntary_installment_details AS (
  SELECT
    ...
    CASE
      WHEN transaction_snapshot_installment_details.period = 1 THEN ROUND(ROUND((1 / 100) * charges.amount,2) - ROUND((1 / 100) * transaction_snapshot_installment_details.add_ons,2),2)
      ELSE ROUND((1 / 100) * charges.amount,2)
...
order_items.is_cancelled items_cancel,
  ...
  WHERE 
    ...
    --AND multic.transaction_id IS NULL
),
--------------------------------------------------------------------------------------------------------
...
order_items.is_cancelled items_cancel
  ...
  WHERE 
    ...
    --AND orders.create_time >= '2025-01-01'
    --AND orders.human_id = 'L77023127' --status <> 'PENDING' 
    AND charges.status NOT IN ('FAILED','PENDING')
...
transformation AS (
  SELECT
    ...
    OrderID,
    OrderItem,
    CASE WHEN InvoiceNo IS NULL AND combine.TransactionStatus = 'SUCCESSFUL' THEN '' ELSE InvoiceNo END AS InvoiceNo,
    CAST(FORMAT_DATE('%d%m%Y', combine.OrderDate) AS STRING) AS OrderDate,
    case 
      WHEN InsuredID='' THEN '-' 
      ELSE InsuredID 
    END AS InsuredID,
  ...
  CASE WHEN sap.U_OrderItem is not NULL THEN
  CAST(FORMAT_DATE('%d%m%Y', DATE(sap.BatchRunDate)) AS STRING) ELSE 
  CAST(FORMAT_DATE('%d%m%Y', combine.BatchRunDate) AS STRING) END
  AS PaymentDate,
  combine.Period,
  TotalPeriods,
-- PendingPayment,-- ผิด 
  ROUND(((((TotalPremium+TotalEIR+TotalSBT+ProcessingFee+ProcessingFeeVat+ShippingFee+ShippingFeeVat)-Discount)-Discount)/TotalPeriods)*(TotalPeriods-combine.Period),2) as PendingPayment,
/*
case 
...
*/
  CASE 
    WHEN InsuranceType = 'MOTOR_TYPE_COMPULSORY' THEN 'RCL-Credit Shell'
    WHEN PaymentMethod = 'CASH' THEN 'RCL-Credit Shell'
    WHEN PaymentMethod = 'QR_CODE' THEN 'RCL-Credit Shell'
    ELSE PaymentMethod 
  END AS PaymentMethod,
  CASE 
    WHEN InsuranceType = 'MOTOR_TYPE_COMPULSORY' THEN 'RCL-Credit Shell'
    WHEN PaymentMethod = 'CASH' THEN 'RCL-Credit Shell'
    WHEN PaymentMethod = 'QR_CODE' AND PaymentChannel = 'RABBIT_LENDING' THEN 'RCL-Credit Shell'
    ELSE PaymentChannel 
  END AS PaymentChannel,

CAST(FORMAT_DATE('%d%m%Y', ExpectedDate) AS STRING) AS ExpectedDate,
    ch.old_human_id RefOrder,
    RefundAmountBeforeFee,
    RefundAmountAfterFee,
    BillingAddress,
    CAST(FORMAT_DATE('%d%m%Y', combine.BatchRunDate) AS STRING) AS BatchRunDate,
items_status,
items_cancel
  FROM combine
  INNER JOIN pacific-plating-282708.careos.cancelled_change_orders ch on OrderID = current_human_id
  left join (SELECT U_OrderID, U_OrderItem, BatchRunDate FROM pacific-plating-282708.sap_integration_v2.SAP_LIVE_FULL WHERE TransactionStatus = 'Cancelled (Change order / Rejected)' ) sap ON U_OrderID = ch.old_human_id
),
--------------------------------------------------------------------------------------------------------
...
SELECT 
  ...
  OrderID,
  OrderItem	,
  -- case when TransactionStatus ='paid' and InvoiceNo='' then OrderItem
  -- else InvoiceNo end as 
  CASE 
  ...
  CASE  
    ...
    ELSE PolicyNo 
  END AS PolicyNo,
  -- case when CAST(EndorsementNo AS STRING) is  null then EndorsementNo else ' ' end as EndorsementNo ,
  -- CAST(EndorsementNo AS STRING) AS EndorsementNo,
  NULL AS EndorsementNo,
  --  CASE WHEN CAST(EndorsementNo AS STRING) is NULL then '' else EndorsementNo end as EndorsementNo,
  ChassisNo,
  LicensePlate,
  GrossPremium,
  StampDuty,
  ...
  -- PaymentChannel	,
  -- case when PaymentMethod is null then '' else PaymentMethod end as PaymentMethod,
  -- case when PaymentChannel is null then '' else PaymentChannel end as PaymentChannel,
  ExpectedDate,
  -- '' as RefOrder	,-
 RefOrder,
  -- case when RefundAmountBeforeFee is null then '' else RefundAmountBeforeFee end as RefundAmountBeforeFee,
  -- case when RefundAmountAfterFee is null then '' else RefundAmountAfterFee end as RefundAmountAfterFee,
  '' AS RefundAmountBeforeFee,
  '' AS  RefundAmountAfterFee,
  -- REPLACE(BillingAddress, ',', ' ') AS BillingAddress	,
  BillingAddress,
  BatchRunDate,
items_status,
items_cancel
...
LEFT JOIN check_order_items 
  ON transformation_xxx.OrderID = check_order_items.OrderID
...
finish AS (
  SELECT
  transformation_xxx.CompanyDB, 
  transformation_xxx.OrderID ,
  transformation_xxx.OrderItem ,
  transformation_xxx.InvoiceNo ,
  transformation_xxx.OrderDate,
  transformation_xxx.InsuredID,
  transformation_xxx.Title,
  transformation_xxx.FirstName,
  transformation_xxx.LastName,
  transformation_xxx.InsurerCode,
  transformation_xxx.InsuranceGroup,
  transformation_xxx.InsuranceType,
  transformation_xxx.InsuranceProduct,
  transformation_xxx.ProductType,
  transformation_xxx.PolicyType,
  transformation_xxx.Endorse,
  transformation_xxx.PolicyDate,
  transformation_xxx.PolicyNo ,
  transformation_xxx.EndorsementNo ,
  transformation_xxx.ChassisNo,
  transformation_xxx.LicensePlate,
  transformation_xxx.GrossPremium,
  transformation_xxx.StampDuty,
  transformation_xxx.VAT,
  transformation_xxx.TotalPremium,
  transformation_xxx.WHT,
  transformation_xxx.TotalEIR,
  transformation_xxx.TotalSBT,
  transformation_xxx.ProcessingFee,
  transformation_xxx.ProcessingFeeVat,
  transformation_xxx.ShippingFee,
  transformation_xxx.ShippingFeeVat,
  transformation_xxx.TotalAmount,
  transformation_xxx.Discount,
  transformation_xxx.TransactionStatus,
  transformation_xxx.SubmissionStatus,
  transformation_xxx.ApprovalStatus,
  transformation_xxx.PaymentStatus,
  transformation_xxx.ExpectedReceived,
  transformation_xxx.ActualReceived,
  transformation_xxx.InterestThisPeriod,
  transformation_xxx.PrincipleThisPeriod,
  CASE
    WHEN (arrange.interest_amount != transformation_xxx.TotalEIR AND transformation_xxx.Period != 1) THEN ABS(ROUND((transformation_xxx.InterestEIRThisPeriod)-(transformation_xxx.TotalSBT/NULLIF((transformation_xxx.TotalPeriods-1),0)),2))
    ELSE transformation_xxx.InterestEIRThisPeriod
  END AS InterestEIRThisPeriod ,
  transformation_xxx.PrincipleEIRThisPeriod,
  transformation_xxx.PaymentDate,
  transformation_xxx.Period,
  transformation_xxx.TotalPeriods,
  transformation_xxx.PendingPayment,
  transformation_xxx.PaymentMethod,
  transformation_xxx.PaymentChannel,
  transformation_xxx.ExpectedDate,
  transformation_xxx.RefOrder,
  transformation_xxx.RefundAmountBeforeFee,
  transformation_xxx.RefundAmountAfterFee,
  transformation_xxx.BillingAddress,
  transformation_xxx.BatchRunDate,
  items_status,
  items_cancel
  FROM transformation_xxx
  LEFT JOIN arrange
  ON transformation_xxx.OrderItem = arrange.OrderItem
