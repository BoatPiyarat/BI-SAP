...
-- Object: sap_data_engineer.sap_dashboard_carepay_fully_paid
...
--  3. Pushed policy_start_date filter into order_items CTE (filter early,
--     before the 9-way join, instead of after)
-- NOT changed (needs confirmation): transaction_snapshots' commented-out
--     `WHERE number_of_installment = 1` filter — left exactly as original
-- ============================================================
...
order_items AS (
  ...
  WHERE policy_start_date >= '2025-01-01'
),
...
onetime_master AS (
  ...
  WHERE
    ...
    AND orders.product = 'products/car-insurance'
    AND transaction_snapshots.number_of_installment = 1
),
