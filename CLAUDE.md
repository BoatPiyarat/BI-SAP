...
- **InvoiceNo is immutable in SAP.** Standard (resolved 2026-07-25 by Boat): new InvoiceNo = raw `third_party_id`/charge id, no prefix — already what the create-flow views do (`sap_dashboard_carepay_installment`). Never invent a `2_`-prefixed InvoiceNo for anything new. Rows already Paid/Cancelled, and the `newpayment`/`cancel` flows that mirror them: pass through whatever SAP already has verbatim (including legacy `2_` prefixes from before this standard) — never reformat an existing value.
