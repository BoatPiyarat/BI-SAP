...
BEGIN
  ...
  INSERT INTO
    `pacific-plating-282708.sap_integration_v3.v3_scenario_activation_approval`
  VALUES(p_flow_key,p_evidence_run_id,p_approval_id,p_approval_reference,p_approved_by,
    p_approved_at,p_expires_at,p_approved_commit,p_scheduler_job_name,p_desired_schedule,
    p_desired_time_zone,p_manual_fallback,CURRENT_TIMESTAMP());
  ASSERT @@row_count=1 AS 'Scenario activation approval registration failed';
END;
...
BEGIN
  ...
  ASSERT SAFE_CAST(JSON_VALUE(p_non_overlap_evidence,'$.enabledJobCount') AS INT64)>0
    AND SAFE_CAST(JSON_VALUE(p_non_overlap_evidence,'$.collisionCount') AS INT64)=0
    ...
    AND JSON_VALUE(p_non_overlap_evidence,'$.activatedJobName')
      =JSON_VALUE(p_scheduler_poststate_json,'$.name')
    AND JSON_QUERY_ARRAY(p_non_overlap_evidence,'$.enabledJobs') IS NOT NULL
...
CREATE OR REPLACE PROCEDURE
  `pacific-plating-282708.sap_integration_v3.sp_claim_v3_scenario_activation`(
    ...
    p_scheduler_resource_json JSON,p_workflow_revision STRING,p_activated_by STRING,
    p_verification_reference STRING)
BEGIN
  ...
  SELECT p_activation_id,p_flow_key,p_evidence_run_id,
    JSON_VALUE(p_scheduler_resource_json,'$.name'),p_scheduler_resource_json,
  ...
  INSERT INTO `pacific-plating-282708.sap_integration_v3.v3_scenario_activation_ledger`
  ...
  SELECT p_activation_id,p_flow_key,p_evidence_run_id,p_approval_id,a.scheduler_job_name,
    a.desired_schedule,a.desired_time_zone,p_workflow_revision,'PRESTATE_CAPTURED',
    CAST(NULL AS JSON),CAST(NULL AS JSON),p_activated_by,CURRENT_TIMESTAMP(),
    p_verification_reference
  FROM `pacific-plating-282708.sap_integration_v3.v3_scenario_activation_approval` a
  WHERE a.flow_key=p_flow_key AND a.evidence_run_id=p_evidence_run_id
    AND a.approval_id=p_approval_id;
  ASSERT @@row_count=1 AS 'Activation ledger claim failed';
  COMMIT TRANSACTION;
END;
