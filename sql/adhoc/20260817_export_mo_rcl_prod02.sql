...
ASSERT (SELECT COUNT(*)
  ...
  WHERE request_id=v_request_id AND gate_status='PASS' AND declared_flow='RCL'
    AND operation='NEWPAYMENT' AND delivery_folder='RCB_MOTOR'
    AND STARTS_WITH(file_name,'INSURANCE_RCB_')
...
ASSERT (SELECT COUNT(*)
  FROM `pacific-plating-282708.sap_integration_v3.mo_rcl_prod02_ready`)=6093
  AS 'ready row count changed after gate';
ASSERT (SELECT COUNT(DISTINCT OrderItem)
  FROM `pacific-plating-282708.sap_integration_v3.mo_rcl_prod02_ready`)=939
...
ASSERT (SELECT TO_HEX(SHA256(STRING_AGG(TO_JSON_STRING(r),'\n'
  ...
  FROM `pacific-plating-282708.sap_integration_v3.mo_rcl_prod02_ready` r)=v_candidate_sha256
